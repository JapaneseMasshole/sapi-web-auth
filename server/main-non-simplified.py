...
class BlpapiManager:
    ...
    async def auth_req(self, token):
        ...
        auth_service = self._session.getService("//blp/apiauth")
        auth_request = auth_service.createAuthorizationRequest("AuthorizationRequest")
        auth_request.set("token", token)
        identity = self._session.createIdentity()

        self._session.sendAuthorizationRequest(auth_request, identity, blpapi.CorrelationId(cur_cid))
